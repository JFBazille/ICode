...
from ICode.estimators.welch import welch_square_loss_l2pen
...
def welch_worker(imgs, masker, regu='off', lbda=1, wtype=1, j1=2, j2=8):
    ...
    if regu=='off':
        return welch_estimator(appro, mask, wtype, j1, j2)

    if regu=='tv':
        return welch_tv_estimator(appro, mask, wtype, j1, j2, lbda)[1]

    if regu=='l2':
        return welch_l2_estimator(appro, mask, wtype, j1, j2, lbda)[1]
...
def welch_estimator(appro, mask=None, wtype=1, j1=2, j2=8):
    """
    ...
    Hurst_exponent, dico = hurstexp_welchper(appro)

    return Hurst_exponent


def welch_tv_estimator(appro, mask=None, wtype=1, j1=2, j2=8, lbda=1):
    """
    This function estimate the Hurst exponent of a signal based
...
def welch_l2_estimator(appro, mask=None, wtype=1, j1=2, j2=8, lbda=1):
    """
    This function estimate the Hurst exponent of a signal based
    on Detrend Fluctuation Analysis.
    -----------------------------------------------------------
    ...
    fg = lambda x, lbda, **kwargs: welch_squared_loss_l2pen(log2frq, log2pwr, Hurst_init, aest,
                                                      compute_energy=True, compute_grad=False)

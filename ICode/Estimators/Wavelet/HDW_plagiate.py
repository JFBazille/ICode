...
# implemented by P. Abry in matlab code
# this Python version yields results that are very cloth the the results yield by
# the original matlab function (see test_plagiate for a comparison and test_preg for a test of several regularity)
import sys
import numpy as np
...
def HDW_p(data,N=4,distn =0,q = np.array(2),j1= 2,j2 =8):
  '''
  This function compute the Hurst exponent of a signal 'data' using Discrete Wavelet transform 
  '''
  
  n = len(data)
  nbvoies = int( np.log2(n) )
  
  #N la regularite d'ondelettes de daubechie
  #distn 0 si on veut prendre en compte une hypothese de donnees gaussienne
  norm = 1 #la norme qui sera utilisee
  printout =0
  sigtype = 0 #on fait comme si tout etait gaussien de toute facon le non gaussien n'est pas implemente
  ...
  wtype =1
  q2 = q + 0  
  sortie = regrespond_det2(dico['Elogmuqj'][0],dico['Varlogmuqj'][0],q2,dico['nj'],j1,j2,wtype)
...
def wtspecq_statlog3(appro,N,norm,q,nbvoies,distn,printout) :
  """
  This function is a Python Clone of the eponyme Matlab function implemented by P. Abry
  ...
  l = list()
  loge = np.log2(np.e)
  #petit if else sur une question de format de q
  #est-ce que l'array contient un ou plusieurs element
  #ca n'a l'air de rien comme ca mais c'est important
  if q.ndim == 0:
    lenq = 1
    tq = True
    qctab = np.zeros(2)
  ...
  nbvoies = min( int(np.log2(n/(2*N+1)))  , nbvoies)#    %   safer, casadestime having problems
  ...
  if distn==0:#   %% GAUSSIEN
	...
	minindex = np.argmin( abs(q[k]-qtab) )#    % find index of closest q value
  ...
  elif  distn==1:  #%% Not-Gaussian, but finite variance,  using asymptotic forms  ~C/nj
  ...
  for j in np.arange(0,nbvoies):#%-- Phase 1, get the coefficients at this scale
    ...
    if nj[j] == 0 and printout >1 :    #                % forget this j and exit if no coefficients left!
    ...
    for  k in np.arange(0,lenq):    #%  Loop on the values of q
      if lenq ==1:
	qc = q + 0
      else:
	qc = q[k] 
      absdqk = abs(decime)**qc        #% the absolute powers are the new base quantities
      logmuqj[k,j] = np.log2(np.mean(absdqk))    #% Calculate  log[ S_q(j) ]    ( in L1 language )
      
      #%%% Now calculate the output quantities:  Elogmuqj  and  Varlogmuqj
      if distn == 0:  #%% GAUSSIEN
	k0 = np.where(qctab[k] == qtab)[0][0]                         #% find index of qctab in qtab (will exist)
	if  nj[j] < Nlim[k0] and printout>1:    #%  if nj too small, need to use tabulation
	  print '! Using tabulated values for |Gaussian|^q distribution:  q,j = %4.1f  %d \n'%(qc,j)
	  minindex = abs(np.log2(nj[j])-np.log2(Ntab)).argmin()# % choose nearest tabulated nj
	...
	else:  #% Large nj(q): For each q,j: Asymptotic iid based analytic expressions#% V = 2( 1+C_4/2 ) = 2 + C_4 = [ E[|d|^2q] / (E[|d|^q)^2 ]  - 1,  can calculate if d Gaussian
	  V  = np.sqrt(np.pi)*gamma(qc+1/2.)/gamma((qc+1)/2.)**2 - 1
	...
	if  parametric: #   % use a parametric model for bias and variance, and fit from data
	  ...
	  if  Varj == 0: # % this can happen, is set by logstat as a sign of not enough data
		  ...
		  Varj = loge**2 * (np.std(absdqk)**2/mean(absdqk)**2 ) /nj[j]#  % estimate C_4 + 2 directly
	  ...
	  V = np.std(absdqk)**2/np.mean(absdqk)**2 #      % estimate  C_4 + 2  directly
    ...
    appro = convolue[nl:njtemp:2]#;% new approximation, filtre meme taille que pour detail=> memes effets de bord
    njtemp = len(appro) 
    del convolue
     
  return {'Elogmuqj':Elogmuqj,'Varlogmuqj':Varlogmuqj,'nj':nj,'logmuqj':logmuqj,'gg1':gg1,'gjj':gjj,'l':l}


##Cette fonction differe de la precedente par la fonction de convolution utilise
##wtspecq_statlog3 utilise celle de numpy tandis que wtspecq_statlog32 utilise celle de
##scipy.ndimage 
##Il y a une petite diffenrece entre les deux je ne saurais pas dire  quoi elle tient
##en revanche les deux on ete testee et renvoie des resultat satisfaisant
##l'avantage de wtspecq_statlog32 est qu'elle accepte des entrees appro matricielles
##La function qui utilise numpy est plus rapide et plus simple a utiliser et a
##une variance legerement plus faible 
def wtspecq_statlog32(appro,N,norm,q,nbvoies,distn,printout) :
  ...
  for j in np.arange(0,nbvoies):#%-- Phase 1, get the coefficients at this scale
    ...
    l.append(convolue)
    decime = convolue[:,nl:njtemp:2] #% details at scale j. decime always becomes empty near the end,
    nj[j] = decime.shape[1]           # % number of coefficients
    if nj[j] == 0 and printout >1 :    #                % forget this j and exit if no coefficients left!
      print 'In wtspecq_statlog, oops!  no details left at scale ' + str(j)+'\n'
      break
	...
	qc = q[k] 
      absdqk = abs(decime)**qc        #% the absolute powers are the new base quantities
      logmuqj[:,k,j] = np.log2(np.mean(absdqk,axis = 1))    #% Calculate  log[ S_q(j) ]    ( in L1 language )
	...
	if  parametric: #   % use a parametric model for bias and variance, and fit from data
	  ...
	  if  Varj == 0: # % this can happen, is set by logstat as a sign of not enough data
		  ...
		  Varj = loge**2 * (np.std(absdqk,axis=1)**2/mean(absdqk,axis=1)**2 ) /nj[j]#  % estimate C_4 + 2 directly
	  Varlogmuqj[:,k,j] = Varj*np.ones(apshape[1]) #  % this is true, an estimate of the Var of the log
	  Elogmuqj[:,k,j]   = LogEj#  % Beware confusing notation, if bias corrected, this is log(E[muqj])...
	else: #%  Asymptotic formula with direct estimate of C_4 from data
	  V = np.std(absdqk,axis=1)**2/np.mean(absdqk,axis=1)**2 #      % estimate  C_4 + 2  directly
	  Varlogmuqj[:,k,j] =  (loge**2 * V /nj[j])*np.ones(apshape[0]) #  % as before, asymptotic formulae
	  gj              = -loge/2 * V /nj[j] 
	  #% Modif PA --- le 11/01/2004 pour forcer gj = 0
	  #% Elogmuqj(k,j) = logmuqj - gj ;
	  Elogmuqj[:,k,j] = logmuqj[:,k,j]
...
def regrespond_det2(yj,varyj,q,nj,j1,j2,wtype):
  ...
  scalemax = len(yj)
  j=np.arange(0,scalemax)#juste pour le plot mais n'est pas implemente
  ...
  #Rmq on est en Python les indices commencent a 0 !
  #Par la suite on utilise jj non comme indice mais comme valeur,
  #on pourrait penser qu'il faille apporter une correction a jj (introduire jj = jj +1)
  #mais en fait ca ne change rien au coefficient directeur seulement le aest 
  J  = len(jj)
  njj  = nj[jj]
  yjj  = yj[jj]
  varyjj= varyj[jj]
  ...
  #essayons un autre aest
  #aest = np.mean(yjj-zetaest * (jj+1))#c'est mieux mais pas encore ca, il faut sans doute pondere la moyenne
  #le b estime est trop petit en valeur abslue !
  #aest4 = (sum(yjj/wvarjj)-zetaest *S1)/S0 # celui la est trop petit !! c'est la meme chose que aest 2
  #on fait la moyenne on verra bien si c'est mieux 
...
def rlistcoefdaub(regu):
  ...
  """
  This function return the coefficient of Daubechie Wavelets of regularity 'regu' after it can be convolve with the signal
  """
  if not (regu<11 and regu>0):
    ...
    if regu==4:
      h=np.array([0.230377813309, 0.714846570553,0.630880767930, -0.027983769417,
		  -0.187034811719, 0.030841381836,0.032883011667, -0.010597401785])
    if regu==5:
      h=np.array([0.160102397974, 0.603829269797,0.724308528438, 0.138428145901,
		  -0.242294887066, -0.032244869585,0.077571493840, -0.006241490213,
		  -0.012580751999, 0.003335725285])
    if regu==6:
      h=np.array([0.111540743350, 0.494623890398,0.751133908021, 0.315250351709,
		  -0.226264693965, -0.129766867567,0.097501605587, 0.027522865530,
		  -0.031582039318, 0.000553842201,0.004777257511, -0.001077301085])
    if regu==7:
      h=np.array([0.077852054085, 0.396539319482,0.729132090846, 0.469782287405,
    ...
    if regu==8:
      h=np.array([0.054415842243, 0.312871590914,0.675630736297, 0.585354683654,
    ...
    if regu==9:
      h=np.array([0.038077947364, 0.243834674613,0.604823123690, 0.657288078051,
    ...
    if regu==10:
      h=np.array([0.026670057901, 0.188176800078,0.52720118932, 0.688459039454,
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#%  This routine takes stationary data X(i) of length n, and studies log(E[X]) and the variance of
#%  Y_n = log[  X^(n)  ]  , where  X^(m)  is the mth level aggregation of X 
#%  (ie the process which averages  X  over blocks of size m).   
#%  Since only one sample value of  Y_n  is available, making the variance impossible
#%  to estimate directly,  the data is split into blocks of different sizes m<=n, and the
#%  mean and the variance of the variables  Y_m = log[ X^(m) ]  are studied as a function of m. 
...
#%  constant j, rather we deal with the linear error in approximating E[log] by log[E] etc ).
#%
#%  log(E[X]):  By assuming and fitting a linear relationship with m of the E[Y_m], we can interpolate
...
#%  E[Y_n] as this is not equal to log(E[X]). If there is insufficient data to split into blocks
#%  a point estimate of E[Y_n] is returned instead.
#%
#%  Var[Y_n]:  again, we assume a parametric form:  Var[Y_n] ~ C/n  , and use
#%  values m<n to estimate C.  The difference is that here the estimation is must
#%  trickier and we must check adaptively to see if we are close to convergence or not. 
#%
#%
...
#%
#%   Input:     X: input data over which estimation is to be performed ( typically X = |dj|^q )
#%       printout: 1:  output some diagnostics, else output nothing except warnings
#%       
#%  Output:   LogE:  Estimated  log(E[X]),  basic quantities satisfying linear regression
#%	      Var:  Estimated  Var[Y_n],   variances for linear regression
#%            Elog:  Estimated    E[Y_n],   for interest, could determine the bias term  
...
def  logstat(X,printout):
  ...
  n = len(X) 
  conv_cutoff = 6
  ndecoupe = min(conv_cutoff,int(np.log2(n/16.)))  #%  don't cut into more than 2^6 blocks , estimate becomes bad.  
  ...
  if ndecoupe < 3:             #%  Not enough data to split into blocks.
      Elog = np.log2(np.mean(X))   #%  Not enough data, use simple point estimate for E[Y_n]  
      LogE = Elog            #%  Can't do better in this case
      Var  = 0              #%  Indicates to calling function that the variance is unestimated
  else:  
    #%%% Collect basic stats for Y_m for a range of m  (sample mean and variance)
    for k in np.arange(0,ndecoupe+1):  
	  blocksize = int(n / 2.**(k-1)) #% pick logarithmically spaced set of m values: m=n to n/2^ndecoupe
	  nk[k] = int(n / (blocksize*1.)) #     % number of whole blocks (ie length of Y_m at this m)
	  XM = np.reshape(X[:nk[k]*blocksize],blocksize,nk[k]) #  % matrix:   blocksize * m
	  m[k] = blocksize #    % store aggregation level corresponding to k
	  means = np.mean(XM)#     % perform aggregation, ie get average over each block  (column)
	  index = np.where(means==0)[0][0]#  % check for nasty case, zeros over a full block!
	  if  len(index)>0:
	    means[index] = np.spacing(1)
	    print 'In "logstat", oops! had to replace a large block of zeros in the series! \n'
	  
	  Y_m = np.log2(means) #      % log of means over a block, this is our series Y_m 
	  moy[k] = np.mean(Y_m) #     % Average of these, estimate of the    E[ Y_m ]
	  var[k] = np.var(Y_m) # %                                    Var[ Y_m ]
    
    
    #%% Take care of  E[Y_m]
    p = np.polyfit(1./m,moy,deg=1) #     %  make linear fit of  moy  versus 1/m
    Elog = np.polyval(p,1./n)#        %  return value from fit evaluated at actual data length, est-E[Y_n]
    LogE = p[1]#                 %  intercept gives prediction from the model, corresponds to looking at est-E[Y_m]
				  #%  for larger and larger m, until it converges to log(E[X]), the ultimate goal.

    #%% Now the variance    Recall:  k=(1,2,3) => m(k)=n*(1,1/2,1/4) => nk=n/m(k)=(1,2,4)
    #% Test and use parametric hypothesis:   Var[Y_m] ~ C/m
    nvar = m * var#              % these would all be C
    nvar = nvar[2:len(nvar)] # % the first two have very few points, discard
    nk = nk[2:]              
    mm = sum(nvar*nk) / sum(nk)# % weighted average to fit C, weights according to the amount of data
    ss = np.std(nvar) 
    index = np.where(abs(nvar-mm)<ss)[0]#% detect those which are too far from the weighted average
    #% mm1 = mean(nvar) ;          % try a simple mean instead -> is bad
    mm2 = np.mean(nvar[index])      #% try a simple mean of the `safe' ones  (currently used)
    Var = mm2/n                  #% apply the model to get  est-Var[Y_n],  using  C = mm2  
	...
	print nvar
	print nvar[index]
	fit = np.polyval(p,1./m) #    % calculate resulting linear fit at original points
	plt.figure(1)
	plt.clf
	plt.subplot(311)
	plt.plot(1./m,moy,'rs-', 1./m,fit,'bo-', 1/n,Elog,'*g') #% plot actual, fit, and value returned
	#grid
	plt.subplot(312)
	#% Look as a function of m to see the convergence to  lim E[Y_m] = log(E[X]) 
	plt.plot(log2(m),moy - p[2],'o-')# % m is logarithmical chosen, so rescale to look (p(2) is the INtercept)
	plt.subplot(313)
	#% Check validity of  est-Var[Y_m] ~ C/m  hypothesis and estimation of C
	plt.plot(np.arange(3,ndecoupe+1),nvar,'bo-',  np.arange(2,ndecoupe+1),mm,'ro-',np.arange(2,ndecoupe+1),mm2,'go-')
	#grid
